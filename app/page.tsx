...
export default function LandingPage() {
  ...
  function handleCloseAuthModal() {
    ...
    setTimeout(() => { setIsAuthModalVisable(false); }, 190);
  }
    ...
    <>
      <nav className="flex justify-between items-center w-screen h-[8vh] px-4 md:px-24 py-1 bg-gray-100">
        ...
        <div className="flex gap-2 text-gray-800">
          <Button onClick={() => handleOpenAuthModal(true)} variant="ghost" className="hidden sm:block">Explore</Button>
          <Button onClick={() => handleOpenAuthModal(true)} variant="ghost" className="hidden sm:block">Write</Button>
          <Button onClick={() => handleOpenAuthModal(false)} variant={"ghost"}>Login</Button>
      ...
      <main className="flex flex-col md:flex-row justify-start w-full md:h-[84vh] relative border-y-[1px] border-black duration-700 font-PT selection:text-gray-200 selection:bg-gray-800">
        <div style={{ backgroundColor: `rgb(${backgroundColor},0.3)` }} className="flex flex-col gap-6 justify-center items-start p-4 py-10 md:px-24 md:border-r-[1px] md:border-black">
          ...
          <Button className="w-36" onClick={() => handleOpenAuthModal(true)}>Start Reading</Button>
        </div>
      ...
      <footer className="flex justify-center items-center h-[8vh] w-screen px-4 border-y-[1px] border-black md:border-none bg-gray-100 overflow-hidden">
        <Button variant="ghost" onClick={() => handleOpenAuthModal(true)}>Blog</Button>
        <Button variant="ghost" onClick={() => handleOpenAuthModal(true)} className="hidden md:block">Write</Button>
        <Button variant="ghost">Terms</Button>
        <Button variant="ghost">Polices</Button>
        <Button variant="ghost" onClick={() => handleOpenAuthModal(false)}>SignIn</Button>
        <Button variant="ghost" onClick={() => handleOpenAuthModal(true)}>Signup</Button>
      </footer>
