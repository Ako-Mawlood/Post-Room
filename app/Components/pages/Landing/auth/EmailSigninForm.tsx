...
const EmailSigninForm = ({ setIsSigninFormVisable }: emailSigninFormPropsType) => {
  const { register, handleSubmit, formState: { errors ,isSubmitting } } = useForm<signinFormDataType>({ defaultValues: { email: "", password: "" } })
  ...
  return (
    <main className="flex flex-col items-center gap-12 w-full sm:w-1/2">
      ...
      <form onSubmit={handleSubmit(handleSignin)} className="flex flex-col items-center gap-2 w-full px-4">
        {errors.root?.message && <p className="w-full bg-red-500/15 border border-red-500 text-red-500 p-2 text-sm font-semibold">{errors.root.message}</p>}

        <label className="flex flex-col items-start gap-2 w-full">
          <span>Email</span>
          {errors.email?.message && <p className="text-red-500 text-xs font-semibold">{errors.email.message}</p>}
        ...
        <label className="flex flex-col items-start gap-2 w-full">
          <span>Password</span>
          {errors.password?.message && <p className="text-red-500 text-xs font-semibold">{errors.password.message}</p>}
          <div className="w-full relative">
            <Input
      ...
      <Button disabled={isSubmitting} onClick={() => setIsSigninFormVisable(false)} variant={"ghost"}><ArrowIcon className="mr-2 size-8" />All sign in options</Button>
    </main>
  )
}
