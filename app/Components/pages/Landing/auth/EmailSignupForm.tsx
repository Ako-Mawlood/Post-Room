...
const EmailSignupForm = ({setUserEmail, isSignupFormVisable, setIsSignupFormVisable, setIsVerifyMessageVisable }: EmailSignupFormPropsType) => {
  const { register, handleSubmit, getValues,setFocus, formState: { errors, isDirty } } = useForm<{ email: string }>({ defaultValues: { email: "" } });
  ...
  return (
    <main className="flex flex-col items-center gap-12 w-full sm:w-1/2">
      ...
      <form onSubmit={handleSubmit(handleSignup)} className="flex flex-col items-center gap-2 w-full px-4">
        {errors.root?.message && <p className="w-full text-red-500 text-sm text-center font-semibold">{errors.root?.message}</p>}
        <label className="flex flex-col items-center gap-2 w-full">
          ...
          {errors.email?.message && <p className="w-full text-red-500 text-sm text-center font-semibold">{errors.email?.message}</p>}
        </label>
        <Button
