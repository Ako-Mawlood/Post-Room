...
const BlogsPreview = ({ sliderIndex, backgroundColor }: BlogsPreviewPropsType) => {
    return (
        <div style={{ backgroundColor: `rgb(${backgroundColor})` }} className="w-full  md:w-[720px] h-[70vh] md:h-auto relative border-y-[1px] border-black md:border-none">
            {
                previewBlogs.map((blog, index) => (
                    <div key={blog.id} className={clsx("h-[70vh] md:h-full w-full absolute top-0 left-0 duration-500", {
                        ...
                        <div className="flex flex-col md:flex-row w-full h-3/5 absolute md:relative">
                            <div
                                ...
                                className="w-full h-full relative bottom-[-2px] left-0 z-40">
                                <Badge
                                    variant="secondary"
                                    className="flex items-center gap-3 absolute md:bottom-[-15px] left-4 mt-8 rounded-none md:rounded-full text-xs opacity-85  z-50">
                                    <Image
                                        className="size-6"
                                        src={starIcon}
                                        alt="Star icon"
                        ...
                        <div className="md:px-5 md:py-7 lg:py-10">
                            <div style={{ backgroundColor: `rgb(${backgroundColor})` }} className="flex flex-col gap-2 w-full md:h-2/5 absolute md:relative bottom-0 left-0 p-4 mb-8 md:m-0 md:p-0 font-semibold z-50">
                                ...
                                <div className="flex items-center gap-3">
                                    <Avatar>
                                        ...
                                        <AvatarFallback> {blog.fullName.split(" ").slice(0, 2).map(word => word[0].toUpperCase())}
                                        </AvatarFallback>
                                    </Avatar>
