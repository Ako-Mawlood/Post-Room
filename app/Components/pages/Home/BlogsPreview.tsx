...
const BlogsPreview = () => {
    ...
    return (
        <section className="flex flex-col md:flex-row items-center justify-start">
            <div className="flex flex-col md:flex-row items-center h-[163vw] sm:h-[143vw] md:h-[42vw] md:w-5/6 mt-0 mx-6 md:mt-6 md:ml-auto overflow-hidden">
                <div style={{ transform: `translateY(${-sliderIndex * 100}%)` }} className={`flex flex-col gap-2 w-full h-full duration-1000 transition-transform transform`}>
                    {previewBlogs.map((blog: blogType, index) => (
                        <div key={blog.id} className={clsx("w-full flex flex-col md:flex-row justify-start items-center", {
                            "opacity-0 duration-1000": sliderIndex != index,
                            "opacity-100 duration-1000": sliderIndex == index,
                        })}>
                            ...
                            <div className="flex flex-col justify-start md:justify-center md:ml-24 lg:ml-32 items-start w-full h-[53vw] sm:h-[30vw] md:w-2/5">
                                <h1 className="text-2xl leading-10 my-2 md:text-[3vw] font-semibold line-clamp-1 md:line-clamp-none">{blog.title}</h1>
                                <p className="text-lg md:text-xl my-2 line-clamp-2 md:line-clamp-none">{blog.description}</p>
                                <Link className="bg-blue-600 text-gray-200 w-36 py-3 text-center rounded-full font-semibold text-xl mt-4 hover:opacity-90" href='/'>Read blog</Link>
                            </div>
                        </div>
                    ))}
            ...
            <div className="flex md:flex-col gap-4 w-3/4 h-4 md:w-1 md:h-72 md:mr-auto mt-3 justify-between items-center">
                ...
                {isOnFocus ?
                    <IoPlayCircleOutline onClick={() => setIsOnFocus(false)} className="text-gray-500 size-28 md:size-11 cursor-pointer" size={40} /> :
                    <IoPauseCircleOutline onClick={() => setIsOnFocus(true)} className="text-gray-500 size-28 md:size-11 cursor-pointer" size={40} />
                }
            </div>
        </section>
